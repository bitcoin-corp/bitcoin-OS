...
1. Click "Claim Task" on https://bitcoin-marketing.vercel.app/contributions#tasks
...
Task #${task.id}`;
  ...
  const command = `gh issue create --repo bitcoin-apps-suite/bitcoin-marketing --title "${title}" --body "${body.replace(/"/g, '\\"').replace(/\n/g, '\\n')}"`;
...
console.log('View them at: https://github.com/bitcoin-apps-suite/bitcoin-marketing/issues');