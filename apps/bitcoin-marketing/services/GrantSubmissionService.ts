import { BitcoinMarketingNFTService } from './NFTService';
import { 
  NFTFile, 
  NFTContent, 
  NFTMetadata, 
...
export class GrantSubmissionService {
  private nftService: BitcoinMarketingNFTService;
  
  constructor() {
    this.nftService = new BitcoinMarketingNFTService();
  }
   ...
   */
  async awardBMarketingTokens(submissionId: string, awardAmount: number, reviewNotes?: string): Promise<GrantSubmission> {
  ...
  async getSubmissionStats(): Promise<{
    ...
    const stats = submissions.reduce((acc, nft) => {
      ...
      totalBMarketingAwarded += bwriterAward;
    ...
    return `
      ...
      <body>
        ...
        <div class="section">
          ...
          <p><span class="label">Platform:</span><span class="value">Bitcoin Marketing</span></p>
